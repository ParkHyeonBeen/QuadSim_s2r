...
class ReplayBufferGym:
    def __init__(self, args, env):
        ...
        self.buffer["state_history"] = np.empty((self.capacity, self.state_dim*self.n_history), dtype=np.float32)
        self.buffer["next_state"] = np.empty((self.capacity, self.state_dim), dtype=np.float32)
        self.buffer["action_history"] = np.empty((self.capacity, self.action_dim*self.n_history), dtype=np.float32)
        self.buffer["reward"] = np.empty((self.capacity, 1), dtype=np.float32)
        self.buffer["done"] = np.empty((self.capacity, 1), dtype=np.float32)
    ...
    def add(self, s, a, r, ns, d):
        ...
        np.copyto(self.buffer["state_history"][self.idx], s)
        np.copyto(self.buffer["action_history"][self.idx], a)
        np.copyto(self.buffer["reward"][self.idx], r)
        np.copyto(self.buffer["next_state"][self.idx], ns)
        np.copyto(self.buffer["done"][self.idx], d)
    ...
    def get_batch(self, batch_size):
        ...
        for key in self.buffer.keys():
            transitions[key] = self.buffer[key][ids].copy()
        return transitions
