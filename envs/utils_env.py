...
sys.path.append(str(Path('utils_env.py').parent.absolute()))  # 절대 경로에 추가
...
def normalize(inp, inp_max, inp_min):

    if type(inp) is not torch.Tensor:
        normal_mat = np.zeros((len(inp), len(inp)))
        np.fill_diagonal(normal_mat,  2 / (inp_max - inp_min))
    else:
        inp_max = torch.tensor(inp_max, dtype=torch.float).cuda()
        inp_min = torch.tensor(inp_min, dtype=torch.float).cuda()
        normal_mat = torch.diag(2 / (inp_max - inp_min))
    normal_bias = (inp_max + inp_min) / 2
    inp = (inp - normal_bias) @ normal_mat
    return inp

def denormalize(inp, inp_max, inp_min):

    if type(inp) is not torch.Tensor:
        denormal_mat = np.zeros((len(inp), len(inp)))
        np.fill_diagonal(denormal_mat, (inp_max - inp_min) / 2)
    else:
        inp_max = torch.tensor(inp_max, dtype=torch.float).cuda()
        inp_min = torch.tensor(inp_min, dtype=torch.float).cuda()
        denormal_mat = torch.diag((inp_max - inp_min) / 2)

    denormal_bias = (inp_max + inp_min) / 2
    inp = inp @ denormal_mat + denormal_bias
